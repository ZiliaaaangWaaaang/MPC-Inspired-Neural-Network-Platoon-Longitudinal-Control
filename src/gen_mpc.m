function [] = gen_mpc(params, data_path)
    ...
    augsys = setmpcsignals(augsys, ...
        'MO', 1:params.Nx, 'UO', params.Nx+1:2*params.Nx); % 线性时不变系统
    
    weights = struct(...
        'MV', sqrt(params.R), ... % mpc_obj.ManipulatedVariables
        'MVRate', 1e-5 * ones(1, params.Nu), ...
        'OV', [sqrt(params.Q), zeros(1, params.Nx)]); % mpc_obj.OutputVariables
    mpc_obj = mpc(augsys, Ts, params.Hp, params.Hc, weights);
    Y = struct('Weight', [zeros(1, params.Nx), ones(1, params.Nx)]);
    U = struct('Weight', sqrt(params.R));
    setterminal(mpc_obj, Y, U); % 设置MPC对象的终端惩罚
